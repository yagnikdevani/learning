...
export default function Album() {
  ...
  return (
    <React.Fragment>
      <CustomizedDialogs isDialogOpened={isOpen} handleCloseDialog={() => setIsOpen(false)}></CustomizedDialogs>
      <CssBaseline />
      <main>
        {/* Hero unit */}
          ...
          <Grid container spacing={4}>
            {cards.map((card) => (
              <Grid item key={card.moduleId} xs={12} sm={6} md={4}>
                ...
                <CardActionArea onClick={() => (card.isModel) ? handleOpen(card.isModel) : '' } href= { (!card.isModel) ? '#' : '' }>
                  <CardMedia
